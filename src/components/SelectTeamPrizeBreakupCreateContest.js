...
import { TouchableOpacity, View, Dimensions, Image ,ActivityIndicator,FlatList} from 'react-native';
import COLORS from './colors';
import CustomText from './CustomText';
import { show, validateWalletNoBonus } from '../utils/Globals';
import { Actions } from 'react-native-router-flux';
import EventBus from 'react-native-event-bus';



export default class SelectTeamPrizeBreakupCreateContest extends Component {
    ...
    fetch(url, {
      ...
      .then((responseJson) => {
        ...
        if (responseJson.status === 1) {
          ...
          var walletD = validateWalletNoBonus(this.props.joiningFee,userObj.wallet.deposits, userObj.wallet.winnings);
      
          var totalSum = walletD.winnings + walletD.bonus + walletD.deposits;
          this.amount_to_play = this.props.joiningFee - totalSum;
...
<View style={{ paddingHorizontal: 8, paddingVertical: 16, width: "100%", height: 400, backgroundColor: "#fff", borderRadius: 3 }}>
          ...
          >
            ...
            {
              this.props.fromJoinContest?<CustomText style={{ color: COLORS.white, fontWeight: "bold", fontSize: 12 }}>
              JOIN CONTEST
          </CustomText>:
